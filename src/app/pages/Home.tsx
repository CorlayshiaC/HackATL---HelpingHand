...
export function Home() {
  ...
  const [personalCampaigns, setPersonalCampaigns] = useState<any[]>([]);
  ...
  const loadFeaturedCampaigns = async () => {
    try {
      ...
      // Get organization campaigns for featured section
      const orgCampaigns = campaigns.filter((c: any) => c.campaignType === 'organization');
      setFeaturedCampaigns(orgCampaigns.slice(0, 3));
      
      // Get urgent campaigns
      const urgent = campaigns.filter((c: any) => c.urgent === true);
      setUrgentNeeds(urgent.slice(0, 3));
      
      // Get personal campaigns
      const personal = campaigns.filter((c: any) => c.campaignType === 'personal');
      setPersonalCampaigns(personal.slice(0, 3));
    } catch (error) {
      console.error('Error loading campaigns:', error);
      // Set empty arrays on error to prevent UI issues
      setFeaturedCampaigns([]);
      setUrgentNeeds([]);
      setPersonalCampaigns([]);
    } finally {
      setLoading(false);
    }
  };
        ...
        <section className="max-w-4xl mx-auto pt-20 pb-16 text-center">
          <div className="mb-6">
            ...
            <p className="text-lg text-gray-600 mb-10 leading-relaxed max-w-2xl mx-auto">
              Discover local causes that matter to you. Our AI
              matches you with community projects based on your
              passions and location — anywhere in the world.
            </p>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <Card className="bg-white border-gray-200">
              <CardHeader>
                ...
                <CardDescription className="text-base leading-relaxed">
                  Answer a few fun questions about what you care
                  about  education, environment, animals, and
                  more.
                </CardDescription>
              </CardHeader>
            </Card>
            ...
            <Card className="bg-white border-gray-200">
              <CardHeader>
                ...
                <CardDescription className="text-base leading-relaxed">
                  Our AI finds local causes that align with your
                  passions and location. It's like a matchmaker
                  for giving.
                </CardDescription>
              </CardHeader>
            </Card>
            ...
            <Card className="bg-white border-gray-200">
              <CardHeader>
                ...
                <CardDescription className="text-base leading-relaxed">
                  Donate as little as $1 to causes you love.
                  Watch your community impact grow over time.
                </CardDescription>
              </CardHeader>
            </Card>
          </div>
                ...
                <Card className="bg-white border-gray-200 hover:shadow-lg transition-all overflow-hidden group cursor-pointer">
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-medium ${getCategoryColor(campaign.category)}`}
                      >
                        {campaign.category}
                      </span>
                    </div>
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-gray-600">
                  These causes need your help right now
                </p>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {urgentNeeds.map((campaign, index) => (
                <Link
                  ...
                  to={`/campaign/${campaign.id}`}
                >
                  <Card className="bg-white border-red-200 border-2 hover:shadow-lg transition-all overflow-hidden group cursor-pointer">
                    <div className="relative h-48 overflow-hidden">
                      <ImageWithFallback
                        src="https://images.unsplash.com/photo-1584739555405-37e477519d5e?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxlbWVyZ2VuY3klMjBzdXBwbGllcyUyMGRvbmF0i29uc3xlbnwxfHx8fDE3NzQ3MjY2NzB8MA&ixlib=rb-4.1.0&q=80&w=1080"
                        alt={campaign.title}
                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                      />
                    ...
                    <CardHeader>
                      ...
                      <div className="space-y-2">
                        ...
                        {(campaign.volunteerNeeds || campaign.goodsNeeded) && (
                          <div className="text-sm text-gray-700 font-medium">
                            {campaign.volunteerNeeds && "Volunteers needed"}
                            {campaign.volunteerNeeds && campaign.goodsNeeded && " • "}
                            {campaign.goodsNeeded && "Goods needed"}
                          </div>
                        )}
                      </div>
                    </CardHeader>
          ...
          <section className="max-w-6xl mx-auto py-16 pb-24">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-gray-600">
                  Personal campaigns from people in your community
                </p>
              </div>
            </div>

            <div className="grid md:grid-cols-3 gap-6">
              {personalCampaigns.map((campaign, index) => (
                <Link
                  key={campaign.id}
                  to={`/campaign/${campaign.id}`}
                >
                  <Card className="bg-white border-gray-200 hover:shadow-lg transition-all overflow-hidden group cursor-pointer">
                    <div className="relative h-48 overflow-hidden">
                      <ImageWithFallback
                        src="https://images.unsplash.com/photo-1763013259233-7437b524ce1b?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxmYW1pbHklMjBwb3J0cmFpdCUyMHNtaWxpbmd8ZW58MXx8fHwxNzc0NzI2NjczfDA&ixlib=rb-4.1.0&q=80&w=1080"
                        alt={campaign.title}
                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                      />
                      <div className="absolute top-4 left-4">
                    ...
                    <CardHeader>
                      ...
                      <div className="mt-2 text-sm text-gray-500">
                        ${campaign.currentAmount.toLocaleString()} raised of ${campaign.goalAmount.toLocaleString()}
                      </div>
                    </CardHeader>
                  </Card>
                </Link>
